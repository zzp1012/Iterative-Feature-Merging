...
def train_epoch(model, dataloader, optimizer, epoch, tfboard_writer):
    ...
    for i,(X, y) in enumerate(dataloader):
        ...
        logit = model(X)
        loss = smooth_crossentropy(logit, y, smoothing=0).mean()
        optimizer.zero_grad()
        loss.backward()
        optimizer.first_step(zero_grad=True)
        # set the second step
        disable_running_stats(model)
        smooth_crossentropy(model(X), y, smoothing=0).mean().backward()
...
if __name__ == '__main__':
    ...
    optimizer = SAM(
        ...
        rho=cfg.train.rho, adaptive=False, lr=cfg.train.lr, 
